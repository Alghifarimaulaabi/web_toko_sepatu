...
export const getTestimoniByProduk = async (req, res) => {
    try {
...
export const getTestimoniByProdukAdmin = async (req, res) => {
    try {
        if (req.user?.role !== 'ADMIN') {
            res.status(403).json({ message: 'Akses ditolak' });
            return;
...
export const replyTestimoni = async (req, res) => {
    try {
        if (req.user?.role !== 'ADMIN') {
            res.status(403).json({ message: 'Akses ditolak' });
            return;
...
//# sourceMappingURL=testimoniController.js.map