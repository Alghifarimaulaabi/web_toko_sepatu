          ...
          <div className="flex flex-col lg:flex-row gap-12 bg-white rounded-3xl p-6 md:p-10 shadow-sm border border-[#D7CCC8]/50">
            ...
            <div className="w-full lg:w-1/2 flex flex-col gap-4">
              ...
              <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide">
                {[1, 2, 3, 4].map((thumb) => (
                  <div key={thumb} className={`relative h-24 w-24 rounded-xl overflow-hidden cursor-pointer border-2 transition ${thumb === 1 ? 'border-[#5D4037]' : 'border-transparent hover:border-[#D7CCC8]'}`}>
                    <Image src={product.image} alt={`Thumbnail ${thumb}`} fill className="object-cover" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex flex-col">
              <div className="flex items-center gap-2 mb-3">
                <span className="bg-[#8D6E63] text-white text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wider">Terlaris</span>
                <div className="flex items-center gap-1 text-sm font-bold text-[#3E2723]">
                  <Star size={16} className="fill-[#FFB300] text-[#FFB300]" />
                  {product.rating} <span className="text-[#8D6E63] font-medium">({product.reviews} ulasan)</span>
                </div>
              </div>
                ...
                <div className="flex gap-3">
                  {product.colors.map((color) => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-full border-4 flex items-center justify-center transition-all ${selectedColor.name === color.name ? 'border-[#8D6E63] scale-110' : 'border-transparent hover:scale-105'} shadow-sm`}
              ...
              <div className="mb-8">
                <div className="flex justify-between items-end mb-3">
                  ...
                  <Link href="#" className="text-sm font-medium text-[#5D4037] hover:underline">Panduan Ukuran</Link>
                </div>
                <div className="flex flex-wrap gap-3">
                  {product.sizes.map((size) => (
                    <button
                      ...
                      className={`w-14 h-12 rounded-xl font-bold text-sm transition-all border ${selectedSize === size ? 'bg-[#5D4037] text-white border-[#5D4037] shadow-md' : 'bg-white text-[#5D4037] border-[#D7CCC8] hover:border-[#8D6E63] hover:bg-[#F5F5F5]'}`}
                    >
                      {size}
                    </button>
                  ))}
